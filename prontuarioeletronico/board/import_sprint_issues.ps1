...
function Get-LabelColor {
    ...
    if ($Label -like "priority:p0") { return "B60205" }
    if ($Label -like "priority:p1") { return "D93F0B" }
    if ($Label -like "epic:*")      { return "5319E7" }
    if ($Label -like "sprint:*")    { return "1D76DB" }
    if ($Label -like "sp:*")        { return "0E8A16" }
    return "1D76DB"
}
